...
- `projectName`: human-readable project name for directory naming
- `phase`: current migration phase (1-based)
- `config.source`: source codebase path and language
- `config.target`: target language, framework, and output path
- `inputFiles`: paths to relevant input files (impact assessment, knowledge base, migration plan, etc.)
- `outputPath`: progress directory (`.aamf/migration/{projectName}`)
- `payload`: additional orchestration state (completedPhases, completedTasks, failedTasks, etc.)
...
4. If parity verification fails → Launch: `failure-recovery`
...
## Sub-Agents (launched via Claude Code CLI)
...
| `failure-recovery` | Handle failures, plan fixes, reduce scope | No |
...
3. Launch `failure-recovery` agent with the failure context.
4. `failure-recovery` will produce a fix plan (potentially with reduced scope).
