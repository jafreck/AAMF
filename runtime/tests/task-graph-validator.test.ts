...
describe('validateTaskGraph', () => {
  ...
  describe('writeRegion validation', () => {
    it('should warn when multiple tasks share a file without writeRegion', () => {
      const tasks = [
        makeTask('task-1', { targetFiles: ['src/codec.rs'] }),
        makeTask('task-2', { targetFiles: ['src/codec.rs'] }),
      ];
      const issues = validateTaskGraph(tasks, { 'task-1': 0, 'task-2': 0 }, 1);
      expect(warningCodes(issues)).toContain('shared-file-no-region');
    });

    it('should error when mixing region and non-region tasks for the same file', () => {
      const tasks = [
        makeTask('task-1', { targetFiles: ['src/codec.rs'], writeRegion: 'types' }),
        makeTask('task-2', { targetFiles: ['src/codec.rs'] }),
      ];
      const issues = validateTaskGraph(tasks, { 'task-1': 0, 'task-2': 0 }, 1);
      expect(errorCodes(issues)).toContain('mixed-region-usage');
    });

    it('should error when duplicate writeRegions target the same file', () => {
      const tasks = [
        makeTask('task-1', { targetFiles: ['src/codec.rs'], writeRegion: 'types' }),
        makeTask('task-2', { targetFiles: ['src/codec.rs'], writeRegion: 'types' }),
      ];
      const issues = validateTaskGraph(tasks, { 'task-1': 0, 'task-2': 0 }, 1);
      expect(errorCodes(issues)).toContain('duplicate-write-region');
    });

    it('should allow distinct writeRegions for the same file', () => {
      const tasks = [
        makeTask('task-1', { targetFiles: ['src/codec.rs'], writeRegion: 'types' }),
        makeTask('task-2', { targetFiles: ['src/codec.rs'], writeRegion: 'compress' }),
        makeTask('task-3', { targetFiles: ['src/codec.rs'], writeRegion: 'decompress' }),
      ];
      const issues = validateTaskGraph(tasks, { 'task-1': 0, 'task-2': 0, 'task-3': 0 }, 1);
      expect(errorCodes(issues)).toHaveLength(0);
    });

    it('should not flag single-task files', () => {
      const tasks = [
        makeTask('task-1', { targetFiles: ['src/codec.rs'] }),
      ];
      const issues = validateTaskGraph(tasks, { 'task-1': 0 }, 1);
